...
class Autowire {
	 ...
	 *
	 * @param string|callable $abstract The bound variable or just an abstract one tha needs to be build.
	 *
	 * @return mixed
	 *
	...
	protected function build( $abstract )
	{
		...
		if ( ! $reflector->isInstantiable() ) {
			throw new Exceptions\NotInstantiable( "{$abstract} could not be instantiated." );
		}
	...
	protected function resolvePrimitive( ReflectionParameter $dependency )
	{
		...
		$message = sprintf(
			"Unresolvable primitive parameter [%s] in class %s",
			$dependency->name,
			$this->dependencyToParamName( $dependency )
		);

		throw new Exceptions\NotInstantiable( $message );
	}
	...
	protected function resolveClass( ReflectionParameter $dependency )
	{
		try {
			...
			throw new Exceptions\NotInstantiable( "Unresolvable class [{$dependency->name}]" );
		}
	}
